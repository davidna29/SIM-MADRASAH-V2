...
- **Modul PPDB Daring:** tabel `ppdb_registrations` (~90 kolom: data siswa A, kesehatan B, berkebutuhan khusus C, alamat siswa D, orang tua E, alamat orang tua F, sekolah asal G, admin-only: kelas/rombel/NIS). Multi-step wizard publik (7 step via Alpine.js) di `/ppdb` tanpa auth — form pendaftaran lengkap dengan validasi (Form Request `PpdbRegistrationRequest` dipakai publik & admin edit). **Saklar buka/tutup + landing page publik:** admin mengatur `ppdb_status` (open/closed) di halaman **Pengaturan PPDB** (`/ppdb/admin/pengaturan`, role `super_admin|tata_usaha|kepala_madrasah`). Saat `closed`, `/ppdb` publik menampilkan **landing page informasi** (`pages.publik.ppdb.landing`) — timeline & alur, syarat (usia/dokumen/kuota), jalur & biaya, kontak panitia, FAQ, countdown ke tanggal buka, dan **form pre-registrasi** (nama + WA) yang disimpan ke tabel **`ppdb_interests`** (unique phone, dedupe via `updateOrCreate`). Saat `open`, `/ppdb` menampilkan form wizard; `store()` ditolak bila tutup (defense in depth). Konten landing disimpan di tabel `settings` berprefix `ppdb_` (`Setting::get/set`, no migrasi baru). Admin routes `/ppdb/admin*` kini **group multi-role** `super_admin|tata_usaha|kepala_madrasah` (sebelumnya tersarang di `role:super_admin` — diperbaiki agar sesuai nav). Controller `Publik\PpdbController` (index + store + interestStore) dan `Ppdb\AdminPpdbController` (index, show, edit, update, accept, reject, exportExcel) serta `Ppdb\PpdbSettingController` (index, update, interestDestroy). Accept workflow: buat Person → Student (tanpa NIS) → Guardian → update status; **NIS & kelas diisi belakangan di modul Data Siswa** (PPDB tidak lagi mengatur NIS/kelas). Export Excel via `Maatwebsite\Excel` mapping kolom EMIS-compatible + 5 kolom link GDrive. Enum reusable: `Pendidikan`, `Pekerjaan`, `Penghasilan`, `Kecamatan`. Migration `guardians.user_id` nullable. Google Drive links untuk dokumen. Fix: label pendidikan/pekerjaan/penghasilan di detail admin. **Simplifikasi (2026):** fitur Generate NIS & Tentukan Kelas **dihapus** (route + method + view + tabel `nis_counters` di-drop via migrasi `000032`); tambah tombol Edit (semua field, semua status); fix tombol Tolak (modal berbagi scope Alpine). **Export Excel mengikuti filter aktif** (`q` + `status` dari halaman index diteruskan ke `PpdbExport`). Test: **38 feature test** di `PpdbModuleTest`. Sidebar "PPDB Daring" kini **parent** di group "Publikasi" (children: Pendaftar + Pengaturan PPDB). Link PPDB di navigasi publik.
...
11. **(BARU) Petunjuk alur di UI** — partial `pages/ppdb/partials/steps.blade.php` (kartu "Alur Pengerjaan Admin" + catatan guardrail per halaman) disertakan di `index`, `show`, `nis-preview`, `assign-class`, dan kini juga di `settings`.
12. **(BARU) Saklar buka/tutup PPDB + landing page publik + pre-registrasi** — admin mengatur di halaman **Pengaturan PPDB** (`ppdb.settings`, role `super_admin|tata_usaha|kepala_madrasah`). Saklar `ppdb_status` (open/closed). Saat `closed`, `/ppdb` menampilkan landing page info (timeline/alur, syarat, jalur, biaya, kontak, FAQ, countdown, form nama+WA); saat `open`, tampil form wizard. `store()` diblokir saat tutup. Data minat di tabel `ppdb_interests` (unique phone, dedupe `updateOrCreate`), dilihat/dihapus di halaman pengaturan. Konten landing = settings berprefix `ppdb_` (SettingSeeder punya default). Admin routes `/ppdb/admin*` dipindah ke group multi-role `super_admin|tata_usaha|kepala_madrasah` (perbaiki inkonsistensi lama yang tersarang di `role:super_admin`).
13. **(BARU) Export Excel mengikuti filter** — tombol "Export Excel" di `pages/ppdb/index.blade.php` kini membawa parameter filter aktif (`q` + `status`); `PpdbExport` menerima argumen baru `$q` (pencarian name/registration_no/nik) selain `status` & `academic_year_id`. `AdminPpdbController::exportExcel` meneruskan `$request->q`. Test `test_export_follows_status_and_search_filter`.
...
- **(BARU) Pengaturan PPDB & landing:** `ppdb_status` default `closed` (SettingSeeder). `SettingSeeder` di-set via `php artisan db:seed --class=SettingSeeder` (atau `migrate:fresh --seed`). Route `/ppdb/admin/pengaturan*` HARUS di bawah wildcard `{registration}`; saklar status hanya kontrol admin (bukan hitung-otomatis dari tanggal). `ppdb_interests.phone` unique — pre-registrasi dedupe per nomor. Landing page aman saat setting kosong (fallback "Segera"/placeholder).
