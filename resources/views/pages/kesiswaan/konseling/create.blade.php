<x-layouts.page
    ...
    <div class="mx-auto max-w-3xl">
        <div class="mb-6">
            <h1 class="text-2xl font-extrabold tracking-tight text-ink sm:text-3xl">Catat Konseling</h1>
            <p class="mt-1.5 max-w-prose text-sm leading-relaxed text-ink-soft">
                Rekam sesi bimbingan dan konseling siswa — tentukan topik, permasalahan, dan tingkat kerahasiaan.
            </p>
        </div>
        ...
        <form method="POST" action="{{ route('konseling.store') }}" enctype="multipart/form-data">
            @csrf
            ...
            <x-ui.sheet>
                <x-ui.form-section title="Data Sesi" description="Pilih siswa dan tentukan jenis konseling.">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <x-ui.select name="student_enrollment_id" id="student_enrollment_id" :options="$students" placeholder="Pilih siswa" />
                        ...
                        <div>
                            ...
                            <x-ui.input type="date" name="session_date" id="session_date" value="{{ old('session_date', now()->format('Y-m-d')) }}" required />
                        ...
                        <div>
                            ...
                            <x-ui.select name="counseling_type" id="counseling_type" :options="$counselingTypes" placeholder="Pilih jenis" />
                ...
                <x-ui.form-section title="Isi Konseling" description="Topik, permasalahan, asesmen, dan tindakan.">
                    <div class="space-y-4">
                        <div>
                            ...
                            <x-ui.input type="text" name="topic" id="topic" value="{{ old('topic') }}" required placeholder="Contoh: Motivasi Belajar" />
                        ...
                        <div>
                            ...
                            <textarea name="problem_description" id="problem_description" rows="3" class="w-full rounded-[var(--radius-control)] bg-sheet px-3.5 py-2.5 text-sm text-ink ring-1 ring-inset ring-rule-strong transition focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Deskripsikan permasalahan siswa…">{{ old('problem_description') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="assessment_result" id="assessment_result" rows="3" class="w-full rounded-[var(--radius-control)] bg-sheet px-3.5 py-2.5 text-sm text-ink ring-1 ring-inset ring-rule-strong transition focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Hasil asesmen/penilaian…">{{ old('assessment_result') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="action_taken" id="action_taken" rows="3" class="w-full rounded-[var(--radius-control)] bg-sheet px-3.5 py-2.5 text-sm text-ink ring-1 ring-inset ring-rule-strong transition focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Tindakan yang sudah dilakukan…">{{ old('action_taken') }}</textarea>
                        ...
                        <div>
                            ...
                            <textarea name="follow_up_plan" id="follow_up_plan" rows="3" class="w-full rounded-[var(--radius-control)] bg-sheet px-3.5 py-2.5 text-sm text-ink ring-1 ring-inset ring-rule-strong transition focus:outline-none focus:ring-2 focus:ring-primary" placeholder="Rencana tindak lanjut…">{{ old('follow_up_plan') }}</textarea>
                            @error('follow_up_plan') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                        </div>
                    </div>
                </x-ui.form-section>

                <x-ui.form-section title="Kerahasiaan & Lampiran" description="Tentukan siapa yang boleh melihat catatan ini.">
                    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
                        <div>
                            ...
                            <x-ui.select name="confidentiality_level" id="confidentiality_level" :options="$confidentialityLevels" :selected="old('confidentiality_level', 'plus_wali_kelas')" />
                            @error('confidentiality_level') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label for="attachment" class="block text-xs font-semibold text-ink-soft mb-1.5">Lampiran</label>
                            ...
                            <p class="mt-1 text-xs text-ink-faint">PDF, JPG, PNG, DOC (maks 5MB)</p>
                ...
                <div class="flex items-center justify-end gap-3 border-t border-rule/70 px-5 py-4 sm:px-6">
                    ...
                    <x-ui.button type="submit" variant="primary" icon="check">Simpan Konseling</x-ui.button>
                </div>
            </x-ui.sheet>
        </form>
    </div>
...
<script>
    document.getElementById('class_group_id').addEventListener('change', function() {
        ...
        const baseUrl = '{{ route("konseling.create") }}';
