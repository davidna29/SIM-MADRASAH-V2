<x-layouts.publik :title="'Mutasi Masuk - Informasi Pendaftaran Pindahan'">
    @php
        $fmt = function ($d) { return $d ? CarbonCarbon::parse($d)->translatedFormat('j F Y') : null; };
        $isOpen = $settings->get('mutasi_status', 'closed') === 'open';
        $buka = $settings->get('mutasi_tanggal_buka', '');
        $tutup = $settings->get('mutasi_tanggal_tutup', '');
        $pengumuman = $settings->get('mutasi_tanggal_pengumuman', '');
        $daftarUlang = $settings->get('mutasi_tanggal_daftar_ulang', '');
        $kuota = $settings->get('mutasi_kuota', '');
        $syarat = collect(explode("
", $settings->get('mutasi_syarat', '')))->map(fn ($x) => trim($x))->filter()->values();
        $kelas = collect(explode("
", $settings->get('mutasi_kelas_tersedia', '')))->map(fn ($x) => trim($x))->filter()->values();
        $biaya = $settings->get('mutasi_biaya', '');
        $wa = $settings->get('mutasi_kontak_wa', '');
        $telp = $settings->get('mutasi_kontak_telepon', '') ?: \App\Models\Setting::get('madrasah_phone', '');
        $jamLayanan = $settings->get('mutasi_jam_layanan', '');
    ...
    <div class="mx-auto max-w-4xl">
        ...
        <section class="text-center">
            ...
            <h1 class="text-3xl font-extrabold tracking-tight text-ink sm:text-4xl">Penerimaan Peserta Didik Pindahan Baru</h1>
            @if ($tahun)
            ...
            <p class="mx-auto mt-3 max-w-prose text-sm leading-relaxed text-ink-soft">
                Layanan penerimaan siswa pindahan masuk (mutasi). Di bawah ini Anda bisa melihat jadwal, syarat,
                dan alur pendaftaran. Ketika dibuka, formulir akan muncul di halaman ini.
            </p>
        </section>
            ...
            <div class="rounded-sheet bg-board text-board shadow-sheet-raised ring-1 ring-inset ring-rule">
                <div class="grid grid-cols-1 gap-6 p-6 sm:p-8 md:grid-cols-2">
                    <div>
                        ...
                        @if ($buka)
                            <div class="mt-2" x-data="mutasiCountdown" data-target="{{ CarbonCarbon::parse($buka)->toIso8601String() }}" x-init="start()">
                                <template x-if="!ready">
                    ...
                    <div class="md:border-l md:border-board-deep/20 md:pl-6">
                        ...
                        <p class="mt-1 text-xs leading-relaxed text-board-ink/70">
                            Tinggalkan nama & nomor WhatsApp Anda. Panitia akan menghubungi saat pendaftaran pindahan dibuka.
                        </p>
                        <form method="POST" action="{{ route('pindahan.interest.store') }}" class="mt-4 space-y-3">
                            @csrf
                            <div>
        ...
        <section class="mt-10">
            ...
            <div class="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
                @foreach ([
                    ['step' => '1', 'title' => 'Isi Data', 'desc' => 'Lengkapi formulir pendaftaran pindahan secara online.', 'icon' => 'pencil-square'],
                    ['step' => '2', 'title' => 'Upload Berkas', 'desc' => 'Unggah scan dokumen yang dibutuhkan, termasuk surat rekomendasi.', 'icon' => 'document-arrow-up'],
                    ['step' => '3', 'title' => 'Verifikasi', 'desc' => 'Panitia memeriksa kelengkapan & keabsahan berkas.', 'icon' => 'check-badge'],
                    ['step' => '4', 'title' => 'Surat Rekomendasi', 'desc' => 'Surat rekomendasi madrasah untuk siswa bersangkutan.', 'icon' => 'envelope'],
                ] as $i => $item)
        ...
        <section class="mt-10">
            ...
            <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                <div class="rounded-sheet bg-sheet p-4 shadow-sheet ring-1 ring-inset ring-rule/60">
                    <p class="flex items-center gap-1.5 text-xs font-semibold text-ink-soft"><x-svg-building-library class="size-4 text-primary" /> Kelas yang Menerima</p>
                    <div class="mt-2 flex flex-wrap gap-1.5">
                        @forelse ($kelas as $k)
                            <span class="rounded-[var(--radius-control)] bg-primary-soft px-2.5 py-1 text-xs font-bold text-primary-strong">{{ $k }}</span>
                        @empty
                            <p class="text-xs text-ink-faint">Menyesuaikan ketersediaan.</p>
                        @endforelse
                    </div>
                </div>
                <div class="rounded-sheet bg-sheet p-4 shadow-sheet ring-1 ring-inset ring-rule/60">
                    <p class="flex items-center gap-1.5 text-xs font-semibold text-ink-soft"><x-svg-folder class="size-4 text-primary" /> Syarat & Dokumen</p>
                    <ul class="mt-2 space-y-1.5">
                        @forelse ($syarat as $s)
                            <li class="flex items-start gap-1.5 text-xs leading-relaxed text-ink">
                                <x-svg-check class="mt-0.5 size-3.5 shrink-0 text-success" /> {{ $s }}
                            </li>
                        @empty
                            <li class="text-xs text-ink-faint">Syarat diumumkan kemudian.</li>
                        @endforelse
                    </ul>
                </div>
            </div>
        ...
        @if ($biaya !== '' || $kuota !== '')
            <section class="mt-10">
                <h2 class="flex items-center gap-2 text-lg font-extrabold tracking-tight text-ink">
                    <x-svg-banknotes class="size-5 text-primary" /> Biaya & Kuota
                </h2>
                <div class="mt-4 grid grid-cols-1 gap-3 md:grid-cols-2">
                    @if ($biaya !== '')
                        <div class="rounded-sheet bg-sheet p-4 shadow-sheet ring-1 ring-inset ring-rule/60">
                            <p class="text-xs font-semibold text-ink-soft">Biaya Pendaftaran</p>
                            <p class="mt-2 text-sm font-bold text-ink">{{ $biaya }}</p>
                        </div>
                    @endif
                    @if ($kuota !== '')
                        <div class="rounded-sheet bg-sheet p-4 shadow-sheet ring-1 ring-inset ring-rule/60">
                            <p class="flex items-center gap-1.5 text-xs font-semibold text-ink-soft"><x-svg-user-group class="size-4 text-primary" /> Kuota</p>
                            <p class="mt-2 text-sm font-bold text-ink">{{ $kuota }} siswa</p>
                        </div>
                    @endif
                </div>
            </section>
